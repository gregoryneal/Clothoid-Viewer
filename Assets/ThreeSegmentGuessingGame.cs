...
namespace Clothoid {
    ...
    public class ThreeSegmentGuesser : MonoBehaviour {
        ...
        void SetupVisuals() {
            ...
            Helpers.DrawOrderedVector3s(new List<Vector3>(){v(start), v(start) + (GetTangent(startAngle) * 3)}, this.startLR);

            endGO.transform.position = new Vector3(end.x, 0, end.y);
            Helpers.DrawOrderedVector3s(new List<Vector3>(){v(end), v(end) + (GetTangent(endAngle) * 3)}, this.endLR);
        }
        ...
        [ContextMenu("N Segments")] 
        public void NSegmentViewer() {
            ClothoidCurve c = new ClothoidCurve();
            float s = sharpness;
            for (int i = 0; i < NSegments; i++) {
                c.AddSegment(new ClothoidSegment(startCurvature, s, arcLength1));
                s = -s;
            }
            Helpers.DrawOrderedVector3s(c.GetSamples(c.Count * 50), this.curveLR);
        }
    }
}