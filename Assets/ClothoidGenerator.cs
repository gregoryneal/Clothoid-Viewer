...
namespace Clothoid {
    ...
    public class ClothoidGenerator : MonoBehaviour {
        ...
        public ClothoidSolutionBertolazzi solutionBertolazzi;
        ...
        private float[] RandomAngles(int num)
        {
            float[] a = new float[num];
            for (int i = 0; i < num; i++)
            {
                a[i] = UnityEngine.Random.Range(-180f, 180f);
            }
            return a;
        }

        private Vector3[] RandomVectors(int num)
        {
            Vector3[] vs = new Vector3[num];
            Vector3 v = new Vector3(UnityEngine.Random.Range(0, 5), 0, UnityEngine.Random.Range(0, 5));
            for (int i = 0; i < num; i++)
            {
                vs[i] = v;
                v += new Vector3(UnityEngine.Random.Range(4, 8), 0, UnityEngine.Random.Range(4, 8));
            }
            return vs;
        }
        ...
        [ContextMenu("Reset BertolazziG1")]
        public void ResetBertolazziG1()
        {
            bertolazzi = new ClothoidCurve();
            //Helpers.DrawOrderedVector3s(bertolazzi.GetSamples(50 * bertolazzi.Count), this.circleLR, 0);
        }


        ClothoidCurve bertolazzi = new ClothoidCurve();
        ...
        public void RedrawCurve() {
            ...
            bertolazzi = ClothoidSolutionBertolazzi.G1Spline(postures);
            Helpers.DrawOrderedVector3s(bertolazzi.GetSamples(50 * bertolazzi.Count), this.circleLR, 0);
