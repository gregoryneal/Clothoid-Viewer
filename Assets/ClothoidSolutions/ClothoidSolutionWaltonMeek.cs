...
namespace Clothoid {
    ...
    public static class ClothoidSolutionWaltonMeek3
    {
        ...
        {
            Vector2 P1 = new((float)x0, (float)z0);
            Vector3 T = ClothoidSegment.RotateAboutAxisRad(Vector3.UnitX, Vector3.UnitY, -t0);
            Vector2 T1 = new(T.X, T.Z);
            Vector2 P2 = new((float)x1, (float)z1);
            T = ClothoidSegment.RotateAboutAxisRad(Vector3.UnitX, Vector3.UnitY, -t1);
            Vector2 T2 = new(T.X, T.Z);

            return G1(P1, T1, P2, T2);
        }

        /// <summary>
        /// The solution to the G1 hermite problem given by Walton & Meek in:
        ...
        /// <param name="P2"></param>
        /// <param name="T2"></param>
        /// <returns>This returns a ClothoidCurve with a single ClothoidSegment.
        /// The reason it doesn't return a ClothoidSegment is because those cannot
        /// be rotated, they are always represented in standard form. The ClothoidCurve
        /// object has the Offset and AngleOffset parameters which are required for
        /// interpolation.</returns>
        /// <exception cref="InvalidClothoidSegmentException"></exception>
        private static ClothoidCurve G1(Vector2 P1, Vector2 T1, Vector2 P2, Vector2 T2, bool reverse = false, bool reflect = false, int recursionDepth = 0)
        {
            Vector2 D = P2 - P1;
            double d = D.Length();
            if (d < TOL)
            {
                throw new InvalidClothoidSegmentException("Too short of segment");
            }
            ...
            if (((phi1 == 0) && (phi2 == Math.PI)) ||
                ((phi1 == Math.PI) && (phi2 == 0)) ||
                ((phi1 == Math.PI) && (phi2 == Math.PI)))
            {
                if (recursionDepth > 20)
                {
                    //we tried too many times lets give up
                    throw new InvalidClothoidSegmentException("ambiguous cases, recursion depth exceeded.");
                }
                else
                {
                    ...
                    return G1(P1, T1, P2, T2, reverse, reflect, recursionDepth + 1);
                }
            }



            type = ClothoidSegment.GetLineTypeFromCurvatureDiff((float)phi1, (float)phi2);
            double k;
            double dk;
            double L;
            double dx = P2.X - P1.X;
            ...
            switch (type)
            {
                ...
                case LineType.LINE:
                    ...
                    break;
                default:
                    FinalParameters f = FitEuler(P1, D / (float)d, d, phi1, phi2, reflect);
                    dk = Math.PI / (f.a * f.a);
                    L = f.a * (f.t2 - f.t1);
                    if (reverse)
                    {
            ...
            ClothoidCurve c = new();
            ClothoidSegment s = new((float)k, (float)dk, (float)L);
            c.Offset = reverse ? new Vector3(P2.X, 0, P2.Y) : new Vector3(P1.X, 0, P1.Y);
            //c.Offset = new Vector3(P1.X, 0, P1.Y).ToUnityVector3();
    ...
    public static class ClothoidSolutionWaltonMeek
    {
        ...
        private static AsymmetricParameters AsymmetricClothoid(Vector2[] points, double tau)
        {
            ...
            Vector2 N0 = new(-T0.Y, T0.X);
            Vector2 N1 = new(-T1.Y, T1.X);
