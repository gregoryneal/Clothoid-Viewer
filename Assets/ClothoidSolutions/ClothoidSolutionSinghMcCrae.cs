...
namespace Clothoid
{
    ...
    public static class ClothoidSolutionSinghMcCrae
    {
        ...
        /// </summary>
        public static ClothoidCurve G2(List<Vector3> polyline)
        {
        ...
        private static Vector3 GetLKPositionAtPolylineIndex(List<Vector3> lkNodes, List<Vector3> polyline, int polylineNodeIndex)
        {
            if (polylineNodeIndex == 0)
            {
                ...
                if (polyline.Count >= 3)
                {
                    ...
                    return new(newArcLength, 0, newCurvature);
                }
                else
                {
                    return new(0, 0, GetLKPositionAtPolylineIndex(lkNodes, polyline, 1).Z);
                }
            }
            if (polylineNodeIndex == polyline.Count - 1)
            {
                ...
                {
                    return new(ClothoidCurve.EstimateArcLength(polyline, polylineNodeIndex), 0, GetLKPositionAtPolylineIndex(lkNodes, polyline, polyline.Count - 2).Z);
                }
            }
            ...
            return new(ClothoidCurve.EstimateArcLength(polyline, polylineNodeIndex), 0, (float)-Clothoid.Mathc.MoretonSequinCurvature(point1, point2, point3));
        }
